...
def create_data_vq(l_vq_model, speakerData_np, listenerData_np, audioData_np,
    ...
    listenerData = Variable(torch.from_numpy(speakerData_np),
                           requires_grad=False).cuda()
...
def load_test_data(args, config, pipeline, tag, out_num=0, vqconfigs=None,
                   smooth=False, speaker=None, segment_tag='', num_out=None, vqvae=False):
    ...
    for speaker in all_speakers:
        ...
        if args.pose:
            print('load pose...')
            tmp_X = p0_deca.astype(np.float32)[:,:,50:53]
            tmp_Y = p0_deca.astype(np.float32)[:,:,50:53]
            tmp_audio = np.load(audio_fp).astype(np.float32)
        else:
            print('load exp...')
            # tmp_X = p0_deca.astype(np.float32)[:,:,:]
            # tmp_Y = p0_deca.astype(np.float32)[:,:,:]
            tmp_X = np.concatenate((p0_deca.astype(np.float32)[:,:,:50], p0_deca.astype(np.float32)[:,:,53:]), axis=2) 
            tmp_Y = np.concatenate((p0_deca.astype(np.float32)[:,:,:50], p0_deca.astype(np.float32)[:,:,53:]), axis=2) 
            tmp_audio = np.load(audio_fp).astype(np.float32)
        if test_X is None:
    ...
    if vqvae:
        preprocess = np.load(os.path.join('./',
        vqconfigs['model_path'],'{}{}_preprocess_core.npz'\
            .format(vqconfigs['tag'], pipeline)))
    else: 
        preprocess = np.load(os.path.join('vqgan/',
...
def load_data(args, config, pipeline, tag, rng, vqconfigs=None, segment_tag='',
              smooth=False):
    ...
    if config['data']['speaker'] == 'all':
        ...
        for speaker in all_speakers:
            tmp_paths, tmp_gt, tmp_quant, tmp_audio, _ = \
                        get_local_files(args, base_dir, speaker, out_num, segment_tag)
        ...
        curr_paths, gt_windows, quant_windows, audio_windows, _ = \
            get_local_files(args, base_dir, config['data']['speaker'],
                            out_num, segment_tag)
    print('===> in/out',
...
def get_local_files(args, base_dir, speaker, out_num, segment_tag):
    ...
    p0_deca = np.load(p0_fp)
    if args.pose == True:
        gt_windows = p0_deca[:,:,50:53]
        quant_windows = np.load(p1_fp)[:,:,50:53]
        audio_windows = np.load(audio_fp)
        app_windows = p0_deca[:,:,56:]
    else:
        gt_windows = np.concatenate((p0_deca[:,:,:50], p0_deca[:,:,53:]), axis=-1)
        quant_windows = np.concatenate((p0_deca[:,:,:50], p0_deca[:,:,53:]),axis=-1)
        audio_windows = np.load(audio_fp)
        app_windows = p0_deca[:,:,56:]

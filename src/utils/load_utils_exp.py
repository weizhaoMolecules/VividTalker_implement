...
def load_test_data(args, config, pipeline, tag, out_num=0, vqconfigs=None,
    ...
    for speaker in all_speakers:
        ...
        tmp_Y = np.load(p1_fp).astype(np.float32)[:,:,:]
        ...
        if not args.pose:
            test_X = np.concatenate((test_X[:,:,:50], test_X[:,:,53:]), axis=2)
            test_Y = np.concatenate((test_Y[:,:,:50], test_Y[:,:,53:]), axis=2)
...
def get_local_files(args, base_dir, speaker, out_num, segment_tag):
    ...
    if args.pose == True:
        ...
        gt_windows = np.concatenate((p0_deca[:,:,:50] ,p0_deca[:,:,53:]),axis=2)
        quant_windows = np.concatenate((p0_deca[:,:,:50] ,p0_deca[:,:,53:]),axis=2)
        audio_windows = np.load(audio_fp)
        app_windows = p0_deca[:,:,56:]

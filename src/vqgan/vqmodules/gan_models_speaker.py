...
import sys
sys.path.append("../model/HeapHeap/Learn2listen/learning2listen-main/src/")
sys.path.append("../")
...
class VQModelTransformer(nn.Module):
    ...
    def get_logit(self, logits, sample=True, filter_value=-float('Inf'),
        ...
        print('logits before [0] {}'.format(logits.shape))
        logits = logits[:,[0],:]
        ...
        if sample:
            ...
            print('probs shape after reshape {}'.format(probs.shape))
            ix = torch.multinomial(probs, num_samples=sample_idx+1)[:,[-1]]
            print('before reshape ix: {}'.format(ix.shape))
            probs = probs.reshape(shape[0],shape[1],shape[2])

...
from vqmodules.gan_models_speaker import setup_vq_transformer, calc_vq_loss
import sys
sys.path.append('../')
from utils.load_utils_exp import *


def generator_train_step(args, config, epoch, generator, g_optimizer, train_X,
                         rng, writer):
    ...
    for bii, bi in enumerate(batchinds):
        ...
        g_loss = calc_vq_loss(args, prediction, gtData, quant_loss)
...
def generator_val_step(args, config, epoch, generator, g_optimizer, test_X,
    ...
    if testLoss < currBestLoss:
        ...
        fileName = config['model_path'] + \
                        '{}{}_{}best.pth'.format(tag, config['pipeline'],epoch)
...
def main(args):
    ...
    train_speaker, test_speaker, _, _, _, _ = \
                    load_data(args, config, pipeline, tag, rng,
                              segment_tag=config['segment_tag'], smooth=True)
    train_X = np.concatenate((train_speaker[:,:seq_len,:],
                              train_speaker[:,seq_len:,:]), axis=0)
    test_X = np.concatenate((test_speaker[:,:seq_len,:],
                             test_speaker[:,seq_len:,:]), axis=0)
    print('loaded speaker...', train_X.shape, test_X.shape)
    disc_factor = 0.0
    for epoch in range(start_epoch, start_epoch + config['num_epochs']):
        ...
        if epoch == start_epoch+config['num_epochs']-1:
            ...
            break
        generator_train_step(args, config, epoch, generator, g_optimizer, train_X,
                             rng, writer)
        currBestLoss, prev_save_epoch, g_loss = \
            generator_val_step(args, config, epoch, generator, g_optimizer, test_X,
                               currBestLoss, prev_save_epoch, tag, writer)
